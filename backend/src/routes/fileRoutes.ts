...
import { vid_like,view,video, user } from "../drizzle/schema";
...
//         //all associated video_like with user
//         //NEED: need to know what the user has liked or not
//         const user_video_like = await db.select().from(vid_like).where(and(eq(userId,vid_like.user_id),eq(vid_like.liked,true)));
//         //all video_like
//         //NEED: compare all videos liked
//         const all_video_like = await db.select().from(vid_like).where(eq(vid_like.liked,true));
//         //all videos viewed (including us ones) 
//         //NEED: subtract from set of recommended videos unless necessary
//         const all_user_viewed = await db.select().from(view).where(eq(userId, view.user_id));
//         //all videos
//         // const all_videos = db.select().from(video);
...
//             user_video_like,
//             all_video_like,
//             viewed_videos: all_user_viewed.map(v => v.video_id),
//             count
//         });
